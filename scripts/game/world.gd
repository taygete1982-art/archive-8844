...
static func build(root: Node2D) -> Dictionary:
    ...
    wall_pm.bounce = 0.5
    wall_pm.friction = 0.3
    walls.physics_material_override = wall_pm
    ...
    if is_instance_valid(el.flip_l) and el.flip_l.has_method("setup"):
        el.flip_l.setup(ball)
    if is_instance_valid(el.flip_r) and el.flip_r.has_method("setup"):
        el.flip_r.setup(ball)
