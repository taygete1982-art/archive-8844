...
static func slingshot(root, pos, rot) -> Dictionary:
	...
	s.size = Vector2(300, 50)
	c.shape = s
	c.position = Vector2(-30, 0)
	f.add_child(c)
	var v = ColorRect.new()
...
static func build(root: Node2D) -> Dictionary:
	...
	for p in [Vector2(150, 700), Vector2(870, 700), Vector2(510, 320)]:
		...
		s.add_child(U.poly(Color(0.8, 0.62, 0.25, 1), [Vector2(0, -28), Vector2(28, 0), Vector2(0, 28), Vector2(-28, 0)]))
		s.add_child(U.poly(Color(0.05, 0.04, 0.03, 1), [Vector2(0, -12), Vector2(12, 0), Vector2(0, 12), Vector2(-12, 0)]))
		root.add_child(s)
		syms.append(s)
	refs.symbols = syms
	...
	for p in [Vector2(510, 800), Vector2(370, 950), Vector2(650, 950)]:
		...
		b.add_child(U.poly(Color(0.35, 0.28, 0.12, 1), U.octagon(22)))
		bumps.append(b)
	refs.bumpers = bumps
