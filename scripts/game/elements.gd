...
static func slingshot(root, pos, rot) -> Dictionary:
    ...
    side.size = Vector2(360, 50)
    side.position = Vector2(-180, -19)
    side.color = Color(0.08, 0.1, 0.08, 1)
    f.add_child(side)
    var v = ColorRect.new()
    v.size = Vector2(360, 50)
    v.position = Vector2(-180, -25)
    v.color = Color(0.22, 0.26, 0.22, 1)
    f.add_child(v)
    var edge = U.poly(Color(0.22, 0.26, 0.22, 1), [Vector2(-180, -25), Vector2(180, -25), Vector2(180, -17), Vector2(-180, -17)])
    f.add_child(edge)
    var area = Area2D.new()
...
static func flipper(root, pos, side) -> AnimatableBody2D:
    ...
    var side_pts := [Vector2(0, -11), Vector2(250, -4), Vector2(250, 16), Vector2(0, 23)]
    if side < 0:
        base_pts = U.mirror(base_pts)
        vis_pts = U.mirror(vis_pts)
        edge_pts = U.mirror(edge_pts)
        side_pts = U.mirror(side_pts)
    f.add_child(U.poly(Color(0.08, 0.1, 0.08, 1), side_pts))
    f.add_child(U.poly(Color(0.12, 0.14, 0.12, 1), base_pts))
    f.add_child(U.poly(Color(0.3, 0.34, 0.3, 1), vis_pts))
    f.add_child(U.poly(Color(0.85, 0.65, 0.2, 1), edge_pts))
    return f
...
static func build(root: Node2D) -> Dictionary:
    ...
    for p in [Vector2(150, 700), Vector2(870, 700), Vector2(510, 320)]:
        var s = Node2D.new()
        s.position = p
        s.add_child(U.poly(Color(0.2, 0.8, 0.6, 1), [Vector2(0, -28), Vector2(28, 0), Vector2(0, 28), Vector2(-28, 0)]))
        s.add_child(U.poly(Color(0.05, 0.07, 0.06, 1), [Vector2(0, -12), Vector2(12, 0), Vector2(0, 12), Vector2(-12, 0)]))
        root.add_child(s)
        syms.append(s)
    refs.symbols = syms
    ...
    thread.width = 4
    thread.default_color = Color(0.35, 0.6, 0.35, 0.7)
    ...
    for p in [Vector2(250, 550), Vector2(770, 550)]:
        ...
        c.shape = cs
        lv.add_child(c)
        lv.add_child(U.poly(Color(0.3, 0.34, 0.3, 1), U.octagon(30)))
        lv.add_child(U.poly(Color(0.2, 0.8, 0.6, 1), U.octagon(12)))
        lvs.append(lv)
    refs.lovers = lvs
    var bumps := []
    for p in [Vector2(510, 800), Vector2(370, 950), Vector2(650, 950)]:
        ...
        c.shape = cs
        b.add_child(c)
        b.add_child(U.poly(Color(0.2, 0.8, 0.6, 1), U.octagon(22)))
        b.add_child(U.poly(Color(0.85, 0.65, 0.2, 1), U.octagon(10)))
        bumps.append(b)
    refs.bumpers = bumps
